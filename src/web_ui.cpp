...
String WebUi::pageStatus(const BarrierController& barrier, const AppConfig& cfg, const String& ip, uint32_t uptime_ms, const Logger& logger) {
  ...
  s += "</table><h3>Last events</h3><pre>" + logger.recentEvents(20) + "</pre>";
  s += "</body></html>";
  return s;
}
...
String WebUi::pageLogicSettings(const AppConfig& cfg) {
  ...
  s += "Relay logic<select name='relay_active_high'><option value='0'" + String(!cfg.logic.relay_active_high ? " selected" : "") + ">Active LOW</option><option value='1'" + String(cfg.logic.relay_active_high ? " selected" : "") + ">Active HIGH</option></select><br>";
